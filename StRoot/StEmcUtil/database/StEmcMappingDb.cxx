...
StEmcMappingDb::StEmcMappingDb(int date, int time) 
    : mBemcTTable(NULL), mBprsTTable(NULL), mSmdeTTable(NULL), mSmdpTTable(NULL), 
...
void StEmcMappingDb::SetDateTime(int date, int time) {
    if(mChain) {
        ...
        unsigned unixTime = TUnixTime::Convert(TDatime(date,time), true);
	if (mBemcTable && ((mBemcTable->getBeginTime() < unixTime) || (mBemcTable->getEndTime() >= unixTime))) {
    	    mBemcDirty = true;
	}
	if (mBprsTable && ((mBprsTable->getBeginTime() < unixTime) || (mBprsTable->getEndTime() >= unixTime))) {
    	    mBprsDirty = true;
	}
	if (mSmdeTable && ((mSmdeTable->getBeginTime() < unixTime) || (mSmdeTable->getEndTime() >= unixTime))) {
    	    mSmdeDirty = true;
	}
	if (mSmdeTable && ((mSmdeTable->getBeginTime() < unixTime) || (mSmdeTable->getEndTime() >= unixTime))) {
    	    mSmdpDirty = true;
...
void StEmcMappingDb::SetFlavor(const char *flavor, const char *tablename) {
    if(mChain) {
        LOG_ERROR << "StEmcMappingDb::SetFlavor is illegal in a chain" << endm;
            ...
            if((!tablename || !strcmp(tablename, "bemcMap")) && mBemcTable && strcmp(mBemcTable->getFlavor(), flavor))  {
                mBemcTable->setFlavor(flavor);
                mBemcDirty = true;
            }
            if((!tablename || !strcmp(tablename, "bprsMap")) && mBprsTable && strcmp(mBprsTable->getFlavor(), flavor)) {
                mBprsTable->setFlavor(flavor);
                mBprsDirty = true;
            }
            if((!tablename || !strcmp(tablename, "bsmdeMap")) && mSmdeTable && strcmp(mSmdeTable->getFlavor(), flavor)) {
                mSmdeTable->setFlavor(flavor);
                mSmdeDirty = true;
            }
            if((!tablename || !strcmp(tablename, "bsmdpMap")) && mSmdpTable && strcmp(mSmdpTable->getFlavor(), flavor)) {
